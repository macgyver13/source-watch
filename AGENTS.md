...
Refresh is opt-in. Template `wrangler.jsonc` has `triggers.crons: []`. To schedule, set the expression there (example `17 2 * * *` = 02:17 UTC daily). The Worker POSTs GitHub `workflow_dispatch` for `.github/workflows/refresh-feed.yml`. That workflow runs the collector and ingests; it commits nothing. If `GITHUB_DISPATCH_REPO` or `GITHUB_DISPATCH_TOKEN` is empty, cron writes `refresh_skipped` and `/admin` shows `refresh not configured`.

Cron changes need `npx wrangler deploy` or `npx wrangler triggers deploy`; version uploads do not replace triggers. If the old cadence continues, delete the trigger then add the new expression. Do not add GitHub `on.schedule` in addition to Worker cron.
